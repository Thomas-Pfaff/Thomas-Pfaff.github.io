...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("x", "x", min = minX, max = maxX, value = minX, step = 0.1),
      textOutput("slope"),
        tags$head(tags$style("#slope{color: #437fca; font-size: 20px;font-style: bold; }"))
    ),
...
server <- function(input, output, session) {
  ...
  curve(temp[[i]](x), minX, maxX, xlab="", ylab="",
    ...
    points(input$x, temp[[i]](input$x), col = "red",
           cex = 2, pch = 16)
...
## Add derivative graph

  curve(fD, minX, maxX, col = alpha("white",0.0), xlab = "",
        ylab = "", xaxs = "i", cex.axis = 1.5) 
    curve(fD, minX, input$x, lwd = 3, add = TRUE)
    abline(h = 0, lwd = 1.5, col = "black")	
    grid(NULL, NULL, col = "black") 
    title(ylab = "Derivative Graph f'(x)", cex.lab = 1.5, line = 4)
    title(xlab = "x", cex.lab = 1.5, line = 4)
    points(input$x, fD(input$x), col = MyBlue, cex = 2, pch = 16)
    segments(input$x, 0, input$x, fD(input$x),
             lty = 2, col = MyBlue, lwd = 2)
    segments(minX, fD(input$x), input$x,fD(input$x),
             lty = 2, col = MyBlue, lwd = 2)
    axis(1, line = 2, at = input$x, label = input$x, col.axis = MyBlue,
         cex.axis = 1.5, tcl = 2, lwd.ticks = 3, col.ticks = MyBlue)
    axis(2, line = 2, at = fD(input$x), label = round(fD(input$x), 2),
         col.axis = MyBlue, cex.axis = 1.5, tcl = 2,
         lwd.ticks = 3, col.ticks = MyBlue)
  })
